...
data_hp <- read.csv("house_prices.csv", sep=",")
data_hp<- kNN(data_hp,variable=c("LotFrontage","FireplaceQu","MasVnrArea"),k=8)
data_hp <- subset( data_hp, select = -c(LotFrontage_imp,FireplaceQu_imp,MasVnrArea_imp) )
xs<-colnames(data_hp)
xs

intercept_only <- lm(SalePrice ~1, data=data_hp)

#define model with all predictors
all <- lm(SalePrice ~ ., data=data_hp)
